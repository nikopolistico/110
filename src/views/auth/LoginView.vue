...
const handleSubmit = async (formType) => {
  if (formType === 'login') {
    try {
      ...
      if (formFields.rememberMe) {
        localStorage.setItem('email', formFields.loginEmail)
        localStorage.setItem('password', formFields.loginPassword)
      } else {
      ...
      showSnackbar(`Login error: ${error.message}`, 'error')
    }
    ...
    try {
      ...
      showSnackbar('Signup successful, please check your email!', 'success')
      router.push('/home')
      ...
      showSnackbar(`Signup error: ${error.message}`, 'error')
    }
  }
}
...
onMounted(() => {
  ...
  if (savedEmail && savedPassword) {
    formFields.loginEmail = savedEmail
    formFields.loginPassword = savedPassword
